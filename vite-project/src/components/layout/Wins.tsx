...
const showToastOk = (message: string) => {
  M.toast({
    html: `<i class="material-icons left">check</i><span>${message}</span>`,
    displayLength: 2500,
    inDuration: 500,
    outDuration: 500,
    classes: 'alert-ok'
...
const Wins: FC<WinsProps> = ({ id, name, description, rewardsPoints, difficulty, image, verified }) => {
  ...
  const handleButtonClick = async () => {
    ...
    fetch(`http://localhost:8080/users/${idUser}/assign-win/${id}`, requestOptions)
      ...
      .then((result) => {
        console.log(result);
        // Actualiza el estado isVerified si la operación es exitosa
        // showToastOk('¡Felicidades! Has empezado un objetivo');
        if (verified == true){
          alert("Ya has empezado este objetivo anteriormente")
        }
        else{
      ...
      <div className="card card2" style={{ paddingRight: "10px", marginLeft: "-10px", marginRight: "10px", width: "390px", height: "600px", borderRadius: '15px', float: 'left', backgroundImage: `url(${image})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <div className="card-content">
          <div style={{ height: "550px" }}>
            <span className="card-title activator white-text text-darken-4" style={{ height: "100%", textShadow: '2px 2px 4px rgba(1, 1, 1, 1)' }}>{name}{verified}<i className="material-icons right"></i></span>
          </div>
